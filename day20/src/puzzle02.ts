...
let emptyLine: boolean[];
let algorithm: boolean[] = [];
let image: boolean[][] = [];
linereader.eachLine("./input/input.txt", (line, last) => {
    if (algorithm.length === 0) {
        algorithm = line.split("").map(binary => binary === "#");
    } else if (line.length > 0) {
        let imageLine: boolean[] = [false, false].concat(line.split("").map(value => value === "#")).concat([false, false]);
        if (image.length === 0) {
    ...
    if (last) {
        ...
        let printImage = (image: boolean[][]) => {
            image.forEach(line => console.log(line.map(value => value ? "#" : ".").join("")));
            console.log("");
        };

        let processImage = (image: boolean[][]): boolean[][] => {
