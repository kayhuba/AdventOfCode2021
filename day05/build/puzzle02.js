...
line_reader_1.default.eachLine("./input/input.txt", function (line, last) {
    ...
    if (last) {
        ...
        var drawLineSegment_1 = function (segment, coordinateSystem) {
            ...
            var width = segment.end.x - segment.start.x;
            var y = segment.start.y;
            var height = segment.end.y - segment.start.y;
            var maxSteps = Math.max(width, height);
            for (var step = 0; step <= maxSteps; step++) {
                var x_1 = segment.start.x + Math.floor(width * step / maxSteps);
                var y_1 = segment.start.y + Math.floor(height * step / maxSteps);
                coordinateSystem[y_1][x_1]++;
            }
        };
